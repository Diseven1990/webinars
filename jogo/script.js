...
function renderFinalRanking(entry) {
  let rankings = saveRankingEntry(entry).map((item) => ({ ...item, isCurrent: item.id === entry.id }));
  const nationalPos = rankingPosition(rankings, entry.id);
  const centerPos = rankingPosition(rankings, entry.id, entry.center);
  const badge = badgeForScore(entry.correct);
    ...
    <main class="ranking-shell">
      <section class="result-summary">
        ...
        <div class="summary-grid">
          <div><span>Acertos</span><strong>${entry.correct}/${entry.total}</strong></div>
          <div><span>Tempo</span><strong>${formatTime(entry.timeSeconds)}</strong></div>
        </div>
        <p class="position-copy">A tua posição: <strong>#${nationalPos} Nacional</strong> · <strong>#${centerPos} ${escapeHtml(entry.center)}</strong></p>
      </section>
  ...
  function setFilter(filter) {
    filtersEl.querySelectorAll('button').forEach((btn) => btn.classList.toggle('active', btn.dataset.filter === filter));
